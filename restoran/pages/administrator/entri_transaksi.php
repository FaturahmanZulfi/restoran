<?php 
  ...
  $sql = "SELECT oder.id_oder,oder.id_user,oder.tanggal,masakan.nama_masakan,masakan.harga,detail_oder.qty,oder.status_oder FROM oder INNER JOIN detail_oder USING (id_oder) INNER JOIN masakan USING(id_masakan) WHERE oder.status_oder = 'belum dibayar'";
  ...
  if(ISSET($_POST['bayar'])){
    ...
    $sql = "UPDATE oder SET status_oder = 'sudah dibayar' WHERE id_oder='$id_oder'";
    $exe = mysqli_query($koneksi,$sql);
    if($exe){
      $sql = "UPDATE detail_oder SET status_detail_oder = 'sudah dibayar' WHERE id_oder='$id_oder'";
      $exe = mysqli_query($koneksi,$sql);
      if($exe){
        $sql = "INSERT INTO transaksi VALUES (null,'$id_user','$id_oder',CURRENT_TIMESTAMP,'$total')";
        $exe = mysqli_query($koneksi,$sql);
        if($exe){
...
<head>
  ...
  <title>Kasir Restoran | Entri Transaksi</title>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              <h3 class="box-title">Selesaikan Transaksi Pemesanan</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body table-responsive no-padding">
              <table class="table table-hover">
                ...
                <tr>
                  ...
                  <td><?= $res['harga'] ?></td>
                  <td><?= $res['qty'] ?></td>
                  <?php $total= $res['harga']*$res['qty']?>
                  <td><?= $total ?></td>
                  <td><span class="label label-danger"><?= $res['status_oder'] ?></span></td>
                  <td>
                    <form method="POST">
                      <input type="hidden" name="id_oder" value="<?= $res['id_oder'] ?>">
                      <input type="hidden" name="id_user" value="<?= $res['id_user'] ?>">
                      <input type="hidden" name="total" value="<?= $total ?>">
                      <input type="submit" class="btn btn-block btn-success btn-sm" name="bayar" value="Bayar">
                    </form>
                    <form method="POST">
                      <input type="hidden" name="id_oder" value="<?= $res['id_oder'] ?>">
                      <input type="submit" class="btn btn-block btn-danger btn-sm" name="batal" value="Batal">
                    </form>
                  </td>
                </tr>
                <?php 
